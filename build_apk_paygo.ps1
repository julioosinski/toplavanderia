...
try {
    ...
    if ($androidHome) {
        ...
        Write-Host "⚠️  ANDROID_HOME não configurado. Usando configuração padrão." -ForegroundColor Yellow
    }
} catch {
    Write-Host "⚠️  Android SDK não configurado. Usando configuração padrão." -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "📋 Últimas linhas do log de erro:" -ForegroundColor Yellow
    Get-Content "app\build\outputs\logs\debug.log" -Tail 20 -ErrorAction SilentlyContinue
