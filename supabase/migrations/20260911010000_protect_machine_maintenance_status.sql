-- Impede que confirmação ESP32 / cron / service_role derrube manutenção.
-- O painel (JWT autenticado) continua podendo sair da manutenção para 'available'.

CREATE OR REPLACE FUNCTION public.protect_machine_maintenance_status()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF OLD.status IS DISTINCT FROM 'maintenance' THEN
    RETURN NEW;
  END IF;
  IF NEW.status IS NOT DISTINCT FROM 'maintenance' THEN
    RETURN NEW;
  END IF;

  -- Nunca iniciar ciclo a partir de manutenção (liberação remota / confirm ON).
  IF NEW.status = 'in_use' OR NEW.status = 'running' THEN
    NEW.status := 'maintenance';
    RETURN NEW;
  END IF;

  -- Sair da manutenção só pelo painel autenticado (não pelo ESP32/service_role).
  IF NEW.status = 'available' AND auth.uid() IS NULL THEN
    NEW.status := 'maintenance';
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS trg_protect_machine_maintenance ON public.machines;
CREATE TRIGGER trg_protect_machine_maintenance
  BEFORE UPDATE OF status ON public.machines
  FOR EACH ROW
  EXECUTE FUNCTION public.protect_machine_maintenance_status();

-- Recria admin_remote_release com recusa explícita (não enfileira pulso em manutenção).
