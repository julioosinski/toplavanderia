...
-- Tabela de autorizações de liberação por operador
CREATE TABLE public.operator_release_permissions (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.operator_release_permissions TO authenticated;
GRANT ALL ON public.operator_release_permissions TO service_role;

ALTER TABLE public.operator_release_permissions ENABLE ROW LEVEL SECURITY;
...
USING (
  ...
  OR public.has_role(auth.uid(), 'admin', laundry_id)
);

CREATE POLICY "Admin/super admin manage permissions"
ON public.operator_release_permissions
...
CREATE TRIGGER update_operator_release_permissions_updated_at
BEFORE UPDATE ON public.operator_release_permissions
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Uso corrente de liberação manual do operador (dia/mês em America/Sao_Paulo)
CREATE OR REPLACE FUNCTION public.get_operator_release_usage(_user_id UUID, _laundry_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT (
    auth.uid() = _user_id
    OR public.is_super_admin(auth.uid())
    OR public.has_role(auth.uid(), 'admin', _laundry_id)
  ) THEN
  ...
  SELECT COALESCE(SUM(ROUND(total_amount * 100)::INTEGER), 0)
  INTO _day_cents
  FROM public.transactions
  WHERE user_id = _user_id
    AND laundry_id = _laundry_id
    AND payment_method = 'manual_release'
  ...
  RETURN jsonb_build_object(
    'can_release', COALESCE(_perm.can_release, false),
    'day_cents', _day_cents,
    'month_cents', _month_cents,
    'day_limit_cents', _perm.daily_limit_cents,
    'month_limit_cents', _perm.monthly_limit_cents
  );
END;
$$;

-- admin_remote_release: aplicar autorização e limites para operadores
...
BEGIN
  ...
  UPDATE public.machines SET status = 'running', updated_at = now() WHERE id = _machine.id;
