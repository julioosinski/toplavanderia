  ...
  const scheduleOta = async () => {
    ...
    if (!/^v?\d+\.\d+\.\d+([\w.-]+)?$/i.test(version)) {
      toast({
        ...
        description: "Use o formato v2.2.6 ou v1.3.4-toplav-poltrona",
        variant: "destructive",
      });
      return;
    }
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          A primeira instalação com suporte OTA ainda exige upload USB (lavadoras/secadoras v2.2.6+, poltrona v1.3.4+, café v1.1.0+).
        </CardDescription>
      </CardHeader>
