...
export const ESP32FirmwareOta = () => {
  ...
  const [firmwareVersion, setFirmwareVersion] = useState("v2.2.9");
  ...
  const scheduleOta = async () => {
    ...
    if (!/^v?\d+\.\d+\.\d+([\w.-]+)?$/i.test(version)) {
      toast({
        ...
        description: "Use o formato v2.2.9 ou v1.3.7-toplav-poltrona",
        variant: "destructive",
      });
      return;
    }
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          A primeira instalação com suporte OTA ainda exige upload USB (lavadoras/secadoras v2.2.9+, poltrona v1.3.7+, café v1.1.0+).
        </CardDescription>
      </CardHeader>
